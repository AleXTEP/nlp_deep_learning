...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    l1_lin = np.dot(data, W1) + b1
    l1_sigm = sigmoid(l1_lin)
    
    scores = np.dot(l1_sigm, W2) +  b2
    probs = softmax(scores)
    
    ### END YOUR CODE
    
    cost = np.sum(-np.log(probs[labels ==1]))
    ...
    gradW2 = np.dot(l1_sigm.T, dscores)
    gradb2 = np.sum(dscores, axis=0, keepdims=True)
    
    dhidden = np.dot(dscores, W2.T)
    
    dhidden *= l1_sigm*(1-l1_sigm)
