...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        xi = xavier_weight_init()
        W = (xi((self.config.n_features * self.config.embed_size , self.config.hidden_size)))
        U = (xi((self.config.hidden_size, self.config.n_classes)))

        b1 = tf.Variable(tf.zeros(self.config.hidden_size))
        b2 = tf.Variable(tf.zeros(self.config.n_classes))


        h = tf.nn.relu(tf.add(tf.matmul(tf.to_float(x), tf.to_float(W)), tf.to_float(b1)))
        h_drop = tf.nn.dropout(h, keep_prob = self.dropout_placeholder)
        pred = tf.add(tf.matmul(tf.to_float(h_drop), tf.to_float(U)),  tf.to_float(b2))
