...
def softmax(x):
    ...
    ### YOUR CODE HERE
    score_max_sample = tf.reduce_max(x, axis = 1, keep_dims = True)
    sample_norm = tf.subtract(x , score_max_sample)
    
    exp_scores = tf.exp(sample_norm)
    exp_scores_sum  = tf.reduce_sum(exp_scores, axis = 1, keep_dims=True)
    
    out = tf.div(exp_scores, exp_scores_sum)
...
def cross_entropy_loss(y, yhat):
    ...
    probs_yhat = tf.reduce_sum(tf.multiply(yhat, tf.to_float(y)), axis = 1)
    out = - tf.reduce_sum(tf.log(probs_yhat))  
